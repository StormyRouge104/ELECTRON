...
# КОНЦОВКА
        ...
        d = {'format': 'bestaudio', 'extract_flat':True, 'cookiefile': 'cookies.txt', 'extractor_args': {'youtube': {'player_client': ['web']}}}
        def get_url():
            try:
      ...
      #queues[interaction.guild.id].append(idk2)
        ...
        def get_stream(web_url):
            if not web_url.startswith("http"):
                return web_url
            try:
                youtuberdl= yt_dlp.YoutubeDL({'format': 'bestaudio', 'noplaylist':True, 'cookiefile': 'cookies.txt', 'extractor_args': {'youtube': {'player_client': ['web']}}})
